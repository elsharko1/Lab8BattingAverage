...
public class BattingStatistics {
    ...
    public static void main(String[] args) {
        ...
        while(choice.equalsIgnoreCase("y")) {
            atBats = numBatsEntered(scan);

            int[] arrSize = new int[atBats];
            //System.out.println(arrSize.length);
            basesEarned(scan, arrSize);

            batSlugsNum(atBats, arrSize);
...
/*Jagged 2D example
    ...
    private static void batSlugsNum(int atBats, int[] arrSize) {
    ...
    private static void basesEarned(Scanner scan, int[] arrSize) {
        for (int i = 0; i < arrSize.length; i++) {
    ...
    private static int numBatsEntered(Scanner scan) {
        int atBats;
